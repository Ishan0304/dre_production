"""Baseline EHR modeling pipeline with conservative leakage checks."""
...
from core.contracts import ArtifactRecord, RunArtifactType
...
class BaselineModelConfig:
    """Configuration for conservative baseline EHR model training."""

    target_col: str = "likely_dre"
    ...
    model_filename: str = "ehr_baseline_model.joblib"
    metrics_filename: str = "ehr_baseline_metrics.json"
    notes: str | None = None
...
class LeakageCheckResult:
    """Structured result of conservative feature leakage inspection."""
...
class BaselineTrainingResult:
    """Structured output for one baseline training run."""
...
class EHRBaselinePipeline:
    """Train and evaluate an auditable baseline model for EHR patient tables."""

    def __init__(self, config: BaselineModelConfig | None = None) -> None:
        self.config = config or BaselineModelConfig()

    def run(self, df: pd.DataFrame) -> BaselineTrainingResult:
        """Run validation, leakage checks, training, evaluation, and saving."""

        notes = self.validate_input(df)
        if self.config.notes:
            notes.append(self.config.notes)

        leakage_result = self.run_leakage_checks(df)
        feature_cols = self.select_feature_columns(df, leakage_result)
        train_df, val_df, test_df, split_notes = self.build_splits(df)
        ...
        artifact_records = self._artifact_records(model_path, metrics_path)
    ...
    def validate_input(self, df: pd.DataFrame) -> list[str]:
        ...
        if self.config.patient_id_col not in df.columns:
            raise ValueError(f"Missing patient identifier column: {self.config.patient_id_col}")
        if df[self.config.target_col].isna().all():
            raise ValueError(f"Target column is all null: {self.config.target_col}")
        if df.empty:
            raise ValueError("Cannot train baseline model on an empty dataframe.")
        return []

    def run_leakage_checks(self, df: pd.DataFrame) -> LeakageCheckResult:
        """Inspect candidate features and exclude identifiers, leakage, and text."""

        candidate_cols = list(self.config.feature_cols) if self.config.feature_cols else list(df.columns)
        ...
        leakage_cols = set(LEAKAGE_SENSITIVE_COLUMNS)
        leakage_cols.add(self.config.target_col)
        ...
        if excluded_for_leakage:
            notes.append("target-derived leakage-sensitive columns were excluded")
        if excluded_for_non_numeric:
            notes.append("non-numeric columns were excluded for this baseline")

        return LeakageCheckResult(
            candidate_feature_cols=list(dict.fromkeys(candidate_cols)),
    ...
    def select_feature_columns(
        self,
        df: pd.DataFrame,
        leakage_result: LeakageCheckResult,
    ) -> list[str]:
        """Select final safe numeric feature columns."""

        excluded = set(leakage_result.excluded_feature_cols)
        excluded.update(leakage_result.excluded_for_identifier)
        excluded.update(leakage_result.excluded_for_leakage)
        excluded.update(leakage_result.excluded_for_non_numeric)
        ...
        if not feature_cols:
            raise ValueError("No safe numeric feature columns remain after leakage checks.")
        return feature_cols

    def build_splits(self, df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, list[str]]:
    ...
    def evaluate_model(
        ...
        """Evaluate a fitted model on one split without crashing on undefined metrics."""
    ...
    def save_outputs(self, model: Pipeline, metrics: dict[str, Any]) -> tuple[str | None, str | None]:
    ...
    def _split_randomly(
        ...
        stratify_train_val = self._stratify_or_none(train_val_df[self.config.target_col], notes)
    ...
    def _artifact_records(
        ...
        if model_path is not None:
            records.append(
                ArtifactRecord(
                    artifact_name="ehr_baseline_model",
                    artifact_type=RunArtifactType.MODEL,
                    relative_path=str(Path(model_path).name),
                    description="Fitted baseline EHR classifier.",
                    created_by="EHRBaselinePipeline",
                )
            )
        if metrics_path is not None:
            records.append(
                ArtifactRecord(
                    artifact_name="ehr_baseline_metrics",
                    artifact_type=RunArtifactType.JSON,
                    relative_path=str(Path(metrics_path).name),
                    description="Baseline EHR model metrics.",
                    created_by="EHRBaselinePipeline",
                )
            )
        return records
